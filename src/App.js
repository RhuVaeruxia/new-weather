...
export function PureComponent({ gpt }) {
  const formatYAxisTick = (value) => Math.min(value, 100); // 습도확률은 최대 100을 넘으면 안되기에 고정 처리
  return (
    <div>
      <h4>기온</h4>
      ...
      <ResponsiveContainer width="100%" height={200}>
        ...
        >
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="name" />
          {/* 데이터 불어올시 최대값을 표기하여 예로 들면 4% 이면 4가 최대라서 100처럼 보이는 오해소지가 생겨 100으로 고정하기 위해 별도 설정 */}
